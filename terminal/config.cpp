...
void Config::LoadSettings(Config::TerminalSettings_t s)
{
	...
	if(i<0){
		i=0;
		ui->checkCustomBaud->setEnabled(true);
...
{
	if(ui->checkCustomBaud->checkState() == Qt::Checked){
		settings.serial.baud = ui->editBaudrate->text().toInt();
	}else{
...
/// Enable or disable custmo baudrate checkbox
void Config::on_checkCustomBaud_clicked(bool checked)
{
	ui->editBaudrate->setEnabled(checked);
	ui->boxBaudrate->setEnabled(!checked);
}
